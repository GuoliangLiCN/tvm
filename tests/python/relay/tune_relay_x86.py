...
def tune_kernels(tasks,
    ...
    for i, tsk in enumerate(tasks):
        ...
        # do tuning
        n_trial=1#len(task.config_space)
        print("n_trials is {}".format(n_trial))
...
def tune_and_evaluate(tuning_opt):
    ...
    with autotvm.apply_graph_best(graph_opt_sch_file):
        print("Compile...")
        # TODO (yongwwww): build_module is for graph_runtime specifically
        # need to add autotvm in BuildModule.compile() in vm.py
        with relay.build_config(opt_level=3):
            #graph, lib, params = relay.build_module.build(
            #    net, target=target,  params=params)
            # print("net is\n {}\ngraph is\n {}\nlib is\n{}".format(net, graph, lib))
            ...
            # result = ex.evaluate()(data_tvm, **params)
            build_mod = relay.vm.BuildModule()
            vm = build_mod.compile(mod, target)
            vm.init(ctx)
            print("reaching here")
            result = vm.run(data_tvm)

            print("Evaluate Inference result: {}".format(result.asnumpy().astype(dtype)))
